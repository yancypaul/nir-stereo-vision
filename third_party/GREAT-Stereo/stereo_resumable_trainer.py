...
def train(args: argparse.Namespace) -> str:
    ...
    if args.restore_ckpt is not None:
        ...
        if os.path.exists(optimizer_ckpt_path) and os.path.exists(scheduler_ckpt_path):
            ...
            logging.info("Done loading optimizer.")
            logging.info("Loading scheduler...")
            scheduler_ckpt = torch.load(scheduler_ckpt_path)
            scheduler.load_state_dict(scheduler_ckpt)
            logging.info("Done loading scheduler.")
    ...
    if restore_total_step is not None:
        logging.info("Converting device state of Tensors in optimizer...")
        for state in optimizer.state.values():
            for k, v in state.items():
                if isinstance(v, torch.Tensor):
                    state[k] = v.to("cuda")
        logging.info("Done converting device state of Tensors in optimizer.")
